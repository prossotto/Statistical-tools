...
class Annots():
    ...
    def __init__(self, ax, x, y, hue=None, comp=[], data=None):
        ...
        self.template_labels_0 = self.x_label_0 + self.labels # x axis labels plus hue labels
        self.n_p = int(len(self.labels))
        self.n_x = int(len(self.x_labels))
        self.y = y

        self.sub_x, self.sub_x_p = self._n_labels_space(self.n_p) # labels distances and positions

        self.x_position, self.abs_position = self._positions_labels(self.sub_x_p)

        self.table_pos = self.data.sort_values([self.x, self.hue],
                                               key=lambda x: [self.template_labels_0.index(a) for a in x]).groupby(
            [self.x, self.hue], sort=False)
        ...
        self.data_or = [(self.data_or_0[i], self.data_or_0[i + 1]) for i in
                        range(len(self.data_or_0) - 1)]  # This the possible data groups, ordered correctly
    ...
    def _n_labels_space(self, labels):
        """
        IT will calculate the distance between subsets of labels (sub-labels)
        in the graph depending on the amount of sub-labels
        (hue) for each x label depending on being an even or uneven number of sub-labels,
        It will also calculate the position of x-axis labels on the grid

        :param labels:
        :return: sub_x, sub_x_p
        """
        if self.n_p % 2 == 0:
            sub_x = 0.8 / (2 * self.n_p) # Distance between two sub-labels (even amount)
            n_p_0 = self.n_p - 1
            sub_x_p_0 = range(-((n_p_0)), (n_p_0) + 1, 2)
            sub_x_p = [i for i in sub_x_p_0 if i != 0]
        else:
            sub_x = 0.8 / self.n_p # Distance between two sub-labels (uneven amount)
            sub_x_p = list(range(-(self.n_p - 1) // 2, (self.n_p // 2) + 1, 1))

        return sub_x, sub_x_p

    def _positions_labels(self, sub_x_p):
        """
        It will use the positions of the x_axis labels and return the positions of each label on the grid and
        a dictionary of the sub_positions of each sub-set labels form the "hue" to be used on annotation of
        statistical comparison between groups

        :param sub_x_p:
        :return: x_position, abs_position
        """
        x_position = {self.x_labels[i].get_text(): i for i in # each label with its X-axis position
                           range(self.n_x)}  # xticks coordinates of each label order as in figure
        abs_position = {a: {self.labels[i]: sub_x_p[i] for i in range(self.n_p)} for a in
                             self.x_label_0}  # position of each sub x category as in figure
        return x_position, abs_position
    ...
    def testing_var(self, test=ttest_ind):
        ...
        for i in range(self.n_comp):
            self.g1 = self.data[(self.data[self.x].astype("string") == self.comp[i][0][0]) & (
                        self.data[self.hue].astype("string") == self.comp[i][0][1])][self.y]
            self.g2 = self.data[(self.data[self.x].astype("string") == self.comp[i][1][0]) & (
                        self.data[self.hue].astype("string") == self.comp[i][1][1])][self.y]
            try:
    ...
    def anot_adjacent(self, offset=0.007, yy=None, marks=10, y_height_distance=3, significance=False, ext=0,
                      test=ttest_ind):
        ...
        for i in range(self.n_comp):
            ...
            self.x1 = (self.x_position[
                           idx1] + idsubx1 * self.sub_x) + offset  # first element of comparison x_position[idx1]
            self.x2 = (self.x_position[
                           idx2] + idsubx2 * self.sub_x) - offset  # second element of comparison x_position[idx1]
            try:
    ...
    def errorbars(self, trend="_"):
        ...
        self.means = self.data.groupby([self.x, self.hue], sort=False)[self.y].mean().sort_index(
            key=lambda x: [self.template_labels_0.index(a) for a in x])
        self.ss = self.data.groupby([self.x, self.hue], sort=False)[self.y].sem().sort_index(
            key=lambda x: [self.template_labels_0.index(a) for a in x])
